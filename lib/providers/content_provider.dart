...
class ContentProvider extends ChangeNotifier {
  ...
  Future<void> loadNewContent() async {
    ...
    notifyListeners();
    try {
      // Fetch a small subset from the first category as a mock for "New" 
      // since fetching all and sorting can freeze the app.
      if (_vodCategories.isNotEmpty) {
        final movies = await getVodStreams(categoryId: _vodCategories.first.categoryId);
        // Sort by added descending (newest first)
        movies.sort((a, b) => b.added.compareTo(a.added));
        _newMovies = movies.take(15).toList();
      }
      if (_seriesCategories.isNotEmpty) {
        final series = await getSeriesList(categoryId: _seriesCategories.first.categoryId);
        series.sort((a, b) => b.lastModified.compareTo(a.lastModified));
        _newSeries = series.take(15).toList();
      }
    } catch (e) {
      // Handle silently
    } finally {
