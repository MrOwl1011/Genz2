...
class AuthProvider extends ChangeNotifier {
  ...
  String get playlistId => base64Encode(utf8.encode('${_serverUrl}_$_username'));
  ...
  Future<void> _setActivePlaylist(String name, String url, String username, String password) async {
    _playlistName = name;
    _serverUrl = url;
    _username = username;
    _password = password;
  ...
  Future<void> _savePlaylistToList(String name, String url, String username, String password, {String? oldUrl, String? oldUsername}) async {
    final prefs = await SharedPreferences.getInstance();
    ...
    if (oldUrl != null && oldUsername != null) {
      playlists.removeWhere((p) => p['url'] == oldUrl && p['username'] == oldUsername);
    }
    ...
    if (oldUrl != null && oldUsername != null && (oldUrl != url || oldUsername != username)) {
  ...
  Future<void> updatePlaylistName(String url, String username, String newName) async {
    final prefs = await SharedPreferences.getInstance();
    final playlists = await getSavedPlaylists();
  ...
  Future<void> checkAutoLogin() async {
    ...
    if (playlists.isNotEmpty) {
      ...
      if (activeUrl != null && activeUser != null) {
        try {
          targetPlaylist = playlists.firstWhere((p) => p['url'] == activeUrl && p['username'] == activeUser);
        } catch (e) {
      ...
      if (targetPlaylist != null) {
        ...
        try {
          ...
          await _savePlaylistToList(_playlistName, _serverUrl, _username, _password);
        } catch (e) {
  ...
  Future<bool> login(String name, String url, String user, String pass, {String? oldUrl, String? oldUser}) async {
    ...
    try {
      ...
      await _savePlaylistToList(finalName, url.trim(), user.trim(), pass.trim(), oldUrl: oldUrl, oldUsername: oldUser);
