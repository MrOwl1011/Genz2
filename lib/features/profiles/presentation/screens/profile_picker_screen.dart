...
///   AuthRootHandler between a successful Xtream login and
...
///   just calls ProfileProvider.selectProfile() and AuthRootHandler (which
...
///   (the reactive AuthRootHandler swap doesn't apply here since this is a
...
class _ProfilePickerScreenState extends State<ProfilePickerScreen> {
  ...
  void _tapProfile(ProfileEntity profile) {
    if (_manageMode) {
      Navigator.of(context).push(
        MaterialPageRoute(builder: (_) => ProfileEditScreen(existingProfile: profile)),
      );
      return;
    }
          ...
          child: Column(
            children: [
              Align(
                ...
                child: IconButton(
                  icon: Icon(
                    widget.isEmbedded ? Icons.close_rounded : Icons.logout_rounded,
                    color: colors.ink.withValues(alpha: 0.5),
                  ),
                  tooltip: widget.isEmbedded ? 'Close' : 'Log out',
                    ...
                    children: [
                      for (final p in provider.profiles)
                        Stack(
                          ...
                          children: [
                            ProfileAvatarTile(profile: p, onTap: () => _tapProfile(p)),
                            if (_manageMode)
                              Positioned(
                                top: -2,
                                right: -2,
                                child: Container(
                                  ...
                                  decoration: BoxDecoration(color: colors.brandAccent, shape: BoxShape.circle),
                                  child: const Icon(Icons.edit_rounded, color: Colors.black, size: 14),
                                ),
                              ),
                          ],
                        ),
                      if (provider.canCreateMore) _buildAddTile(colors),
                    ],
                  ),
                ),
              const Spacer(),
  ...
  Widget _buildAddTile(AppColors colors) {
      ...
      child: Column(
        ...
        children: [
          Container(
            ...
            child: Icon(Icons.add_rounded, color: colors.ink.withValues(alpha: 0.5), size: 36),
          ),
