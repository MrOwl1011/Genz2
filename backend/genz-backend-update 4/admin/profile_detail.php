...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'clear_history') {
        ...
        $destUsernameRaw = $destCheck->fetchColumn();
        $destLabel = $destUsernameRaw === false ? false : account_label($destUsernameRaw ?: null, $destAccountId);
        ...
        if ($destLabel === false) {
            ...
            if ((int) $destCountStmt->fetchColumn() >= MAX_PROFILES_PER_ACCOUNT) {
                ...
                try {
                    ...
                    $pdo->commit();
                    $flash = 'Profile copied to ' . $destLabel . ' (favorites and history included).';
                } catch (Throwable $e) {
                    $pdo->rollBack();
    ...
    $destSearchStmt = $pdo->prepare(
        "SELECT accounts.account_id, accounts.username,
                (SELECT d.device_name FROM devices d WHERE d.account_id = accounts.account_id
                 ORDER BY d.last_seen_at DESC LIMIT 1) AS latest_device_name
         FROM accounts
         WHERE (username LIKE :q1 OR account_id LIKE :q2
                OR EXISTS (SELECT 1 FROM devices d WHERE d.account_id = accounts.account_id AND d.device_name LIKE :q3))
               AND account_id != :self
...
require __DIR__ . '/includes/layout_start.php';
?>
<p><a href="account_detail.php?id=<?= urlencode($profile['account_id']) ?>">← Back to <?= html_escape(account_label($profile['username'], $profile['account_id'], $profile['latest_device_name'])) ?></a></p>
...
<div class="card">
  ...
  <form method="GET" class="search-bar">
    ...
    <input type="search" name="dest_q" placeholder="Search destination by device name or account ID…" value="<?= html_escape($destQuery) ?>">
      ...
      <table>
        ...
        <tbody>
          <?php if (empty($destResults)): ?>
            <tr><td colspan="2" class="muted">No matching accounts.</td></tr>
          <?php endif; ?>
          <?php foreach ($destResults as $d): ?>
            <?php $dLabel = account_label($d['username'], $d['account_id'], $d['latest_device_name']); ?>
            <tr>
              <td><?= html_escape($dLabel) ?></td>
              <td>
                <form class="inline" method="POST" onsubmit="return confirm('Copy this profile (with its favorites and history) to <?= html_escape(addslashes($dLabel)) ?>?');">
