...
$params = [];
if ($query !== '') {
    // Five distinct placeholder names bound to the same value — PDO with
    ...
    // device_name/profile name are the practical ways to find one now.
    // Profile name is matched the same way device_name is: an EXISTS
    // against a non-deleted profile on the account, not a join, so an
    // account with several profiles still only counts once per match.
    $where[] = '(username LIKE :q1 OR server_url LIKE :q2 OR account_id LIKE :q3
                 ...
                 OR EXISTS (SELECT 1 FROM profiles p WHERE p.account_id = accounts.account_id AND p.deleted_at IS NULL AND p.name LIKE :q5))';
    $params['q1'] = '%' . $query . '%';
    $params['q2'] = '%' . $query . '%';
    $params['q3'] = '%' . $query . '%';
    $params['q4'] = '%' . $query . '%';
...
$listStmt = $pdo->prepare(
    "SELECT accounts.account_id, accounts.username, accounts.created_at, accounts.last_login_at,
...
function accounts_query_string(int $page, string $query): string
{
    return http_build_query(['q' => $query, 'page' => $page]);
}
?>
<h1>Accounts</h1>
...
<form method="GET" class="search-bar">
  <input type="search" name="q" placeholder="Search device name, profile name, or account ID…" value="<?= html_escape($query) ?>">
  <button type="submit" class="btn primary">Search</button>
</form>

<div class="card table-wrap">
  <table>
    <thead><tr><th>Account</th><th>Devices</th><th>Last Login</th><th>Actions</th></tr></thead>
    <tbody>
      <?php if (empty($accounts)): ?>
        <tr><td colspan="4" class="muted">No accounts match.</td></tr>
      <?php endif; ?>
...
<div class="pagination">
  <?php for ($p = 1; $p <= $totalPages; $p++): ?>
    <a class="btn small <?= $p === $page ? 'primary' : '' ?>" href="?<?= accounts_query_string($p, $query) ?>"><?= $p ?></a>
  <?php endfor; ?>
</div>
<?php endif; ?>
