...
// Checked up front so a database without the table gets a page naming the
// file to run, rather than a bare HTTP 500 from the first query.
$hasTable = db()->query(
    "SELECT COUNT(*) FROM information_schema.TABLES
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'monitored_servers'"
)->fetchColumn();
if ((int) $hasTable === 0) {
    ...
    <div class="card">
      ...
      <p>In cPanel, open <strong>phpMyAdmin</strong>, select your database, and run
         <code>backend/sql/migration_003_monitored_servers.sql</code> from the SQL tab.
