    ...
    if ($action === 'suspend') {
        ...
        $stmt = $pdo->prepare(
            'UPDATE device_tokens SET revoked_at = NOW()
             WHERE account_id = :aid AND device_id = :did AND revoked_at IS NULL'
        );
        $stmt->execute(['aid' => $accountId, 'did' => $deviceId]);
        // The device row itself stays (last_seen_at is still useful history)
        // — only its token is revoked, so it drops to local-only until it
        // registers again, exactly like AuthProvider's non-fatal
        // "backend unreachable" path.
        $flash = $stmt->rowCount() > 0 ? 'Device logged out.' : 'No active session for that device.';
        $flashType = $stmt->rowCount() > 0 ? 'success' : 'error';
...
$devicesStmt = $pdo->prepare('SELECT device_id, device_name, platform, last_seen_at,
    (SELECT MAX(revoked_at IS NULL) FROM device_tokens t WHERE t.account_id = devices.account_id AND t.device_id = devices.device_id) AS has_active_token
...
<h1><?= html_escape($accountLabel) ?> <span class="badge <?= $account['status'] === 'active' ? 'active' : 'suspended' ?>"><?= html_escape($account['status']) ?></span></h1>
...
<div class="card table-wrap">
  <table>
    <thead><tr><th>Device</th><th>Platform</th><th>Last Seen</th><th>Session</th><th></th></tr></thead>
    <tbody>
      <?php if (empty($devices)): ?>
        <tr><td colspan="5" class="muted">No devices.</td></tr>
      <?php endif; ?>
      <?php foreach ($devices as $d): ?>
        <?php $hasActiveToken = (bool) $d['has_active_token']; ?>
        <tr>
          ...
          <td><span class="badge <?= $hasActiveToken ? 'active' : 'suspended' ?>"><?= $hasActiveToken ? 'Active' : 'Logged out' ?></span></td>
          <td>
            <?php if ($hasActiveToken): ?>
              <form class="inline" method="POST" onsubmit="return confirm('Log out this device? It will drop to local-only until it registers again.');">
                <input type="hidden" name="csrf_token" value="<?= html_escape(csrf_token()) ?>">
                <input type="hidden" name="device_id" value="<?= html_escape($d['device_id']) ?>">
                <input type="hidden" name="action" value="revoke_device">
                <button type="submit" class="btn small danger">Log Out</button>
              </form>
            <?php endif; ?>
          </td>
        </tr>
